...
PlotterToSvg.prototype._transform = function(chunk, encoding, done) {
  switch (chunk.type) {
    case 'shape':
      this.defs = this.defs.concat(reduceShapeArray(this._id, chunk.tool, chunk.shape))
      break
...
PlotterToSvg.prototype._finishClearLayer = function() {
  if (this._maskId) {
    this.defs.push(createMask(this._maskId, this._maskBox, this._mask))
    this._maskId = ''
    this._maskBox = []
    this._mask = []
    return true
...
PlotterToSvg.prototype._handleNewRepeat = function(offsets, box) {
  ...
  if (blockLayers > (2 - blockMode)) {
    ...
    this.layer = [maskLayer(maskId, layer)]
    this._maskId = maskId
