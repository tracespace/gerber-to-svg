...
describe 'tool macro class', ->
  it 'should identify itself', ->
    ...
    expect( m.name ).to.equal 'MACRONAME'
  ...
  describe 'run method', ->
    ...
    describe 'creating a tool', ->
      ...
      it 'should return a group if there were several primitives', ->
        m = new Macro ['AMNAME', '1,1,8,0,0', '1,1,4,5,5']
        result = m.run 'D10'
        expect( result.pad[0] ).to.containDeep {
          g: { 
            _: [
              { circle: { cx: 0, cy: 0 } }
      ...
      it 'should return the primitive if it is the only one', ->
        ...
        result = m.run 'D10'
        expect( result.pad[0] ).to.containDeep { circle: { cx: 0, cy: 0 } }
  ...
  describe 'run block method', ->
    ...
    describe 'primitive blocks', ->
      it 'should split up a primitive block and pass it to @primitive', ->
        ...
        expect( m.shapes ).to.containDeep [{ circle: {cx: 0, cy: 0, r: 5*factor} }]
  ...
  describe 'primitive method', ->
    ...
    describe 'for circles', ->
      it 'should add a circle to the shapes and the bbox', ->
        ...
        expect( m.shapes ).to.containDeep [ 
          { circle: { cx: 1*factor, cy: 2*factor, r: 2.5*factor } } 
        ]
        expect( m.masks ).to.eql []
        expect( m.bbox ).to.eql [ -1.5*factor, -0.5*factor, 3.5*factor, 4.5*factor ]
    describe 'for vector lines', ->
      it 'should add a vector line to the shapes and bbox', ->
        ...
        expect( m.masks ).to.eql []
        expect( m.bbox ).to.eql [ 1*factor, -1.5*factor, 15*factor, 3.5*factor ]
      it 'should be able to rotate the line', ->
        m.primitive [2, 1, 5, 1, 0, 10, 0, 90]
        expect( m.shapes ).to.containDeep [ { line: { transform: 'rotate(90)' } } ]
        expect( m.bbox ).to.eql [ -2.5*factor, 1*factor, 2.5*factor, 10*factor ]
    describe 'for center rects', ->
      ...
      it 'should be able to rotate the rect', ->
        ...
        expect( m.shapes ).to.containDeep [ { rect: { transform: 'rotate(270)' } } ]
        expect( m.bbox ).to.eql [ -5*factor, -2.5*factor, 5*factor, 2.5*factor ]
    describe 'for lower left rects', ->
      it 'should add a lower left rect to the shapes and box', ->
        ...
        expect( m.masks ).to.eql []
        expect( m.bbox ).to.eql [ -1*factor, -1*factor, 5*factor, 5*factor ]
      it 'should be able to rotate the rect', ->
        m.primitive [22, 1, 5, 10, 0, 0, 180]
        expect( m.shapes ).to.containDeep [ { rect: { transform: 'rotate(180)' } } ]
    ...
    describe 'for outline polygons', ->
      ...
      it 'should be able to rotate the outline', ->
        ...
        expect( m.shapes ).to.containDeep [{ polygon: { transform: 'rotate(-90)' } }]
    ...
    describe 'for regular polygons', ->
      it 'should add a regular polygon to the shapes and bbox', ->
        ...
        expect( m.masks ).to.eql []
        expect( m.bbox ).to.eql [ -2.5*factor, -2.5*factor, 2.5*factor, 2.5*factor ]
      it 'should be able to rotate the polygon if the center is 0,0', ->
        m.primitive [5, 1, 4, 0, 0, 5, 45]
        d = 2.5*factor / Math.sqrt 2
        expect( Math.abs m.bbox[0]+d ).to.be.below 0.000000001
        expect( Math.abs m.bbox[1]+d ).to.be.below 0.000000001
        expect( Math.abs m.bbox[2]-d ).to.be.below 0.000000001
        expect( Math.abs m.bbox[3]-d ).to.be.below 0.000000001
    ...
    describe 'for moirés', ->
      it 'should add a moiré to the shapes and bbox', ->
        ...
        expect( m.bbox ).to.eql [ -11*factor, -11*factor, 11*factor, 11*factor ]
    ...
    describe 'adding more than one shape', ->
      it 'should be able to have a few primitives involved', ->
        ...
        expect( m.shapes.length ).to.equal 3
    describe 'exposure', ->
      it 'should add a mask to only existing shape', ->
        ...
        expect( m.shapes ).to.containDeep [ { circle: { mask: "url(##{maskId})" } } ]
      it 'should group up previous shapes if theres several and mask them', ->
        ...
        expect( m.masks[0].mask._ ).to.containDeep [
          { rect: { 
              x: -5*factor
              y: -5*factor
              width: 14.5*factor
        ...
        expect( m.shapes[0].g ).to.containDeep {
          mask: "url(##{maskId})"
      ...
      it 'should add several clear shapes in a row to the same mask', ->
        ...
        expect( m.shapes.length ).to.equal 1
        expect( m.masks.length ).to.equal 1
        expect( m.masks[0].mask._.length ).to.equal 3
        expect( m.masks[0].mask._ ).to.containDeep [
