...
describe('coordinate parser', function () {
  ...
  it('should parse properly with leading zero suppression', function () {
    FORMAT.zero = 'L'
    expect(parseCoord('X10', FORMAT)).to.eql({x: 0.01})
    expect(parseCoord('Y15', FORMAT)).to.eql({y: 0.015})
    expect(parseCoord('I20', FORMAT)).to.eql({i: 0.02})
    expect(parseCoord('J-40', FORMAT)).to.eql({j: -0.04})
    expect(parseCoord('X1000Y-2000I3J432', FORMAT)).to.eql({x: 1, y: -2, i: 0.003, j: 0.432})
  })
  ...
  it('should parse properly with trailing zero suppression', function () {
    ...
    expect(parseCoord('X0001Y-0002I3J432', FORMAT)).to.eql({x: 0.01, y: -0.02, i: 30, j: 43.2})
  })
