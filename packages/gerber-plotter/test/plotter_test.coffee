...
describe 'Plotter class', ->
  ...
  describe.skip 'operating', ->
    ...
    describe.skip 'making sure format is set', ->
      ...
      it.skip 'should assume notation is absolute if not set on a drill file', ->
        p = new Plotter null, new DrillParser()
        p.units = 'in'
    ...
    describe.skip 'paths', ->
      ...
      it.skip 'should throw an error for unstrokable tool outside region mode', ->
      ...
      describe.skip 'adding an arc to the path', ->
        ...
        it.skip 'should throw an error if quadrant mode has not been specified', ->
          expect( -> p.write {
        ...
        describe.skip 'single quadrant arc mode', ->
          ...
          it.skip 'should warn for impossible arcs and add nothing to the path', ->
            warnings.hook()
        ...
        describe.skip 'multi quadrant arc mode', ->
          ...
          it.skip 'should warn for impossible arc and add nothing to the path', ->
            warnings.hook()
            p.write { set: { mode: 'cw' }, op: {do: 'int', x: 1, y: 1, j:-1 }}
            expect( warnings.unhook() ).to.match /impossible arc/
            expect( p.path ).to.not.contain 'A'
      ...
      describe.skip 'region mode on', ->
        ...
        it.skip 'should not take the tool into account when calculating the bbox', ->
          p.write { set: { region: true } }
          p.write { op: { do: 'int', x: 5, y: 5 } }
  ...
  describe.skip 'finish layer method', ->
    ...
    it.skip 'should add the current items to the group if only one dark layer', ->
      p.finishLayer()
    ...
    describe.skip 'step repeat', ->
      ...
      describe.skip 'with a dark layer', ->
        ...
        it.skip 'should wrap current in a group, copy it, and add it to @group', ->
          p.finishLayer()
